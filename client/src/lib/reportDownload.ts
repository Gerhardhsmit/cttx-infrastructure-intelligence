...
export function buildGeneratedReportMarkdown(input: {
  ...
  return [
    ...
    ``,
    `## 5. ${getReportTemplateSectionTitle("reserve-manager-recommendations")}`,
    ``,
    bulletList(guidance.reserveManagerRecommendations),
    ``,
    `## 6. ${getReportTemplateSectionTitle("cttx-follow-up-workflow")}`,
    ``,
    bulletList(guidance.cttxFollowUpSteps),
    ``,
    `## 7. ${getReportTemplateSectionTitle("decision-pack")}`,
    ``,
