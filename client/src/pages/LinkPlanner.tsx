...
import { buildGeneratedReportHtml, buildGeneratedReportHtmlFileName } from "@/lib/reportDownload";
...
import { Activity, Antenna, CheckCircle2, CloudCog, FileText, Layers3, MapPin, Mountain, RadioTower, RefreshCw, Save, ShieldCheck, Zap } from "lucide-react";
...
export function serializePlannerTopology(input: {
  planName: string;
  plannerState: PlannerState;
  budgets: Map<string, LinkBudget>;
  viableLinkThresholdKm: number;
  routeDecisionExplanation: string;
  totalDistanceKm: number;
  liveDistanceKm: number;
  weakestFadeMarginDb: number;
}): SerializedPlannerTopology {
  const { planName, plannerState, budgets, viableLinkThresholdKm } = input;
  return {
    planName: planName.trim() || "CTTX LOS Backbone Draft",
    propertyName: plannerState.propertyName,
    totalDistanceKm: input.totalDistanceKm,
    liveDistanceKm: input.liveDistanceKm,
    linkCount: plannerState.links.length,
    uplinkCount: plannerState.links.filter((link) => link.type === "uplink").length,
    backboneCount: plannerState.links.filter((link) => link.type === "backbone").length,
    overThresholdCount: plannerState.links.filter((link) => link.distKm > viableLinkThresholdKm).length,
    weakestFadeMarginDb: input.weakestFadeMarginDb,
    viableLinkThresholdKm,
    routeDecisionExplanation: input.routeDecisionExplanation,
    recommendationSummary: plannerState.recommendationSummary,
    links: plannerState.links.map((link) => {
      const budget = budgets.get(link.id) ?? calculateLinkBudget(link);
      return {
        type: link.type,
        fromName: link.fromName,
        toName: link.toName,
        distKm: link.distKm,
        rslDbm: budget.rslDbm,
        fadeMarginDb: budget.fadeMarginDb,
        outOfRange: link.distKm > viableLinkThresholdKm,
      };
    }),
    highSites: plannerState.highSites.map((site) => ({
      name: site.name,
    ...
    selectedMast: plannerState.selectedMastIndex === null || !plannerState.masts[plannerState.selectedMastIndex] ? null : {
      name: plannerState.masts[plannerState.selectedMastIndex].name,
      provider: plannerState.masts[plannerState.selectedMastIndex].provider,
      closestForProvider: plannerState.masts[plannerState.selectedMastIndex].closestForProvider,
      lat: plannerState.masts[plannerState.selectedMastIndex].lat,
      lng: plannerState.masts[plannerState.selectedMastIndex].lng,
    },
    facilities: plannerState.facilities.map((facility) => ({
      name: facility.name,
      type: facility.type,
      lat: facility.lat,
      lng: facility.lng,
...
export default function LinkPlanner() {
  ...
  const [thresholds, setThresholds] = useState<PlannerThresholds>({ viableLinkThresholdKm: DEFAULT_VIABLE_LINK_THRESHOLD_KM });
  ...
  const selectedMast = plannerState.selectedMastIndex === null ? null : plannerState.masts[plannerState.selectedMastIndex] ?? null;
  const routeDecisionExplanation = useMemo(() => buildRouteDecisionExplanation(plannerState, thresholds.viableLinkThresholdKm), [plannerState, thresholds.viableLinkThresholdKm]);
  ...
  const serializedTopology = useMemo(() => serializePlannerTopology({
    planName,
    plannerState,
    budgets,
    viableLinkThresholdKm: thresholds.viableLinkThresholdKm,
    routeDecisionExplanation,
    totalDistanceKm: totals.totalDistanceKm,
    liveDistanceKm: totals.liveDistanceKm,
    weakestFadeMarginDb: totals.weakestFadeMargin,
  }), [budgets, planName, plannerState, routeDecisionExplanation, thresholds.viableLinkThresholdKm, totals.liveDistanceKm, totals.totalDistanceKm, totals.weakestFadeMargin]);
  ...
  const exportTopologyToReport = () => {
    const reportHtml = buildGeneratedReportHtml({
      audit: {
        id: Date.now(),
        clientName: serializedTopology.propertyName,
        sector: "Remote reserve / operational property",
        propertySizeHa: plannerState.boundaryAreaHa,
        latitude: plannerState.propertyCentre?.lat ?? center.lat,
        longitude: plannerState.propertyCentre?.lng ?? center.lng,
        currentConnectivity: "Link Planner topology exported for CTTX engineering review.",
        linkQuality: `Viable-link threshold ${serializedTopology.viableLinkThresholdKm} km; weakest fade margin ${serializedTopology.weakestFadeMarginDb.toFixed(1)} dB.`,
        primaryArchitecture: "One earned uplink plus nearest-neighbour microwave backbone; no all-to-all LOS spider web.",
        engineeringNotes: serializedTopology.routeDecisionExplanation,
        linkPlannerTopology: serializedTopology,
      },
      observations: [],
      guidance: {
        hasPropertyPin: Boolean(plannerState.propertyCentre),
        mappedObservationCount: 0,
        discoveryGaps: [],
        reportSummary: serializedTopology.recommendationSummary,
        recommendationTone: "Engineering validation recommended before procurement.",
        reserveManagerRecommendations: [
          "Review the exported Link Planner topology with CTTX engineering before quoting or construction.",
          "Validate Fresnel clearance, tower height, power autonomy, and final radio selection on site.",
        ],
        cttxFollowUpSteps: [
          "Confirm the preferred backhaul mast and nearest-neighbour relay sequence.",
          "Convert live/priority links into a survey pack and final RF design after field validation.",
        ],
        decisionPackItems: [
          "Serialized Link Planner topology summary.",
          "Facility list, live-link distance, weakest fade margin, and threshold-risk count.",
        ],
      },
      lead: {
        email: "cttx-engineering@cttx.local",
        company: "CTTX Engineering",
        budget: "Internal Link Planner export",
    ...
    link.href = url;
    link.download = buildGeneratedReportHtmlFileName(serializedTopology.propertyName, Date.now());
    document.body.appendChild(link);
    link.click();
    link.remove();
    URL.revokeObjectURL(url);
    toast.success("Link Planner topology added to a PDF-ready report export");
  };
    ...
    <div className="min-h-screen rounded-3xl bg-slate-950 text-slate-100">
      <section className="grid gap-6 p-5 xl:grid-cols-[420px_minmax(0,1fr)]">
        <aside className="space-y-4">
          ...
          <div className="rounded-3xl border border-white/10 bg-white/[0.04] p-5">
            <div className="grid gap-3">
              ...
              <label className="text-sm font-medium text-slate-200">Viable-link threshold
                <div className="mt-2 grid grid-cols-[1fr_5.5rem] items-center gap-3">
                  <input type="range" min={MIN_VIABLE_LINK_THRESHOLD_KM} max={MAX_VIABLE_LINK_THRESHOLD_KM} step={1} value={thresholds.viableLinkThresholdKm} onChange={(event) => setThresholds({ viableLinkThresholdKm: Number(event.target.value) })} className="h-2 w-full cursor-pointer accent-cyan-300" aria-label="Viable-link threshold in kilometres" />
                  <input type="number" min={MIN_VIABLE_LINK_THRESHOLD_KM} max={MAX_VIABLE_LINK_THRESHOLD_KM} step={1} value={thresholds.viableLinkThresholdKm} onChange={(event) => setThresholds({ viableLinkThresholdKm: Math.min(MAX_VIABLE_LINK_THRESHOLD_KM, Math.max(MIN_VIABLE_LINK_THRESHOLD_KM, Number(event.target.value) || DEFAULT_VIABLE_LINK_THRESHOLD_KM)) })} className="w-full rounded-xl border border-white/10 bg-slate-950 px-3 py-2 text-sm text-white outline-none ring-cyan-400/40 focus:ring-2" aria-label="Viable-link threshold value" />
                </div>
                <span className="mt-1 block text-xs text-slate-400">Report risk threshold only. The topology algorithm remains one uplink plus nearest-neighbour backbone.</span>
              </label>
          ...
          <Panel title="Topology summary" icon={<FileText className="h-4 w-4 text-yellow-300" />}>
            <details open className="rounded-2xl border border-yellow-300/20 bg-yellow-300/10 p-4 text-sm leading-6 text-yellow-50">
              <summary className="cursor-pointer font-semibold text-white">Report-ready snapshot</summary>
              <div className="mt-3 grid grid-cols-2 gap-2 text-xs">
                <SummaryItem label="Links" value={`${serializedTopology.linkCount} total · ${serializedTopology.uplinkCount} uplink · ${serializedTopology.backboneCount} backbone`} />
                <SummaryItem label="Distance" value={`${serializedTopology.totalDistanceKm} km total · ${serializedTopology.liveDistanceKm} km live`} />
                <SummaryItem label="Threshold risk" value={`${serializedTopology.overThresholdCount} over ${serializedTopology.viableLinkThresholdKm} km`} />
                <SummaryItem label="Weakest margin" value={`${serializedTopology.weakestFadeMarginDb.toFixed(1)} dB`} />
              </div>
              <p className="mt-3 text-yellow-50/90">{serializedTopology.routeDecisionExplanation}</p>
            </details>
          </Panel>
        </aside>
        ...
        <main className="space-y-4">
          ...
          <Panel title="LOS-only link budget" icon={<CheckCircle2 className="h-4 w-4 text-emerald-300" />}><div className="overflow-x-auto"><table className="w-full min-w-[920px] text-left text-sm"><thead className="text-xs uppercase tracking-[0.18em] text-slate-400"><tr><th className="py-3">Type</th><th>Path</th><th>Distance</th><th>Bearing</th><th>RSL</th><th>Fade margin</th><th>Fresnel</th><th>Payload</th><th>Status</th></tr></thead><tbody className="divide-y divide-white/10">{plannerState.links.map((link) => { const budget = budgets.get(link.id)!; return <tr key={link.id}><td className="py-3 text-cyan-200">{link.type.toUpperCase()}</td><td>{link.fromName} → {link.toName}</td><td className={link.distKm > thresholds.viableLinkThresholdKm ? "font-semibold text-amber-200" : undefined}>{link.distKm} km{link.distKm > thresholds.viableLinkThresholdKm ? " · over threshold" : ""}</td><td>{budget.bearingDeg}°</td><td>{budget.rslDbm} dBm</td><td className={metricClass(budget.fadeMarginDb)}>{budget.fadeMarginDb} dB</td><td>{budget.fresnelM} m</td><td>{budget.throughputMbps} Mbps</td><td><button type="button" onClick={() => toggleLinkLive(link.id)} className={`rounded-full px-2 py-1 text-xs ${link.distKm > thresholds.viableLinkThresholdKm ? "bg-amber-400/10 text-amber-200" : "bg-emerald-400/10 text-emerald-200"}`}>{link.distKm > thresholds.viableLinkThresholdKm ? `Amber threshold warning · ${thresholds.viableLinkThresholdKm} km` : `Clear LOS · ${link.live ? "live" : "planned"}`}</button></td></tr>; })}</tbody></table></div><details className="mt-4 rounded-2xl border border-cyan-300/20 bg-cyan-300/10 p-4 text-sm leading-6 text-cyan-50"><summary className="cursor-pointer font-semibold text-white">Why this route was chosen</summary><p className="mt-3 text-cyan-50/90">{routeDecisionExplanation}</p></details></Panel>
...
function SummaryItem({ label, value }: { label: string; value: string }) { return <div className="rounded-xl border border-white/10 bg-slate-950/70 p-3"><div className="text-[10px] font-semibold uppercase tracking-[0.18em] text-yellow-200/70">{label}</div><div className="mt-1 text-yellow-50">{value}</div></div>; }
