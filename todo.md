...
- [ ] Add a viable-link threshold slider from 10 km to 25 km, defaulting to 15 km, that recomputes the backbone and warning count when adjusted.
- [ ] Ensure topology rules remain unchanged: one uplink only, nearest-neighbour backbone chain, maximum six links, amber warnings for links over threshold, provider-specific gold-star closest masts, no all-to-all LOS spider web, no auto-connecting remote sites beyond 5 km outside the boundary, and SRTM 10×10 grid as primary high-site source.
- [ ] Make Link Planner output serializable to JSON with high sites, selected mast, backbone links, out-of-range flags, facilities, and threshold settings.
- [ ] Inject Link Planner topology into the existing audit report export with a topology summary table, facility list, and “Cost of Disconnection vs Value of Connected Operations” ROI section.
- [ ] Add or update focused regression coverage for the continuous flow, facility placement, route explanation, threshold recomputation, topology serialization, and report export integration.
- [ ] Validate each priority item with focused tests, TypeScript, build/project health as appropriate, save a checkpoint after each completed priority item, and deliver the final end-to-end handoff.
